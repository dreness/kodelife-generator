            ...
            'files': {
                name: {
                    ...
                    'warnings': [
                        ...
                        for issue in result.issues if issue.severity == 'warning'
                    ],
                    'info': result.info
                }
...
class KlprojAnalyzer:
    ...
    def __init__(self, isf_source_dir: Optional[str] = None, verbose: bool = False):
        ...
        self.glsl_keywords = {
            'void', 'main', 'if', 'else', 'for', 'while', 'do', 'return', 'break', 'continue',
            'in', 'out', 'inout', 'uniform', 'const', 'attribute', 'varying',
            'float', 'vec2', 'vec3', 'vec4', 'mat2', 'mat3', 'mat4', 'int', 'bool',
            'sampler2D', 'samplerCube', 'true', 'false',
            'gl_Position', 'gl_FragCoord', 'gl_FragColor', 'fragColor',
            'texture', 'mix', 'clamp', 'abs', 'fract', 'sin', 'cos', 'dot', 'length',
            'normalize', 'floor', 'ceil', 'mod', 'min', 'max', 'step', 'smoothstep',
            'sqrt', 'pow', 'exp', 'log', 'radians', 'degrees',
            'r', 'g', 'b', 'a', 'x', 'y', 'z', 'w', 's', 't', 'p', 'q',
            'rg', 'gb', 'ba', 'rb', 'xy', 'yz', 'zw', 'xw',
            'rgb', 'rgba', 'xyz', 'xyzw',
            'returnMe', 'co', 'c', 'K', 'pt'  # Common local variable names
        }
    ...
    def check_structure(self, xml_path: Path) -> FileAnalysisResult:
        ...
        try:
            ...
            if params_elem is None:
                ...
                param_names = [p.find('variableName').text for p in params if p.find('variableName') is not None]
    ...
    def _extract_shader_variables(self, shader_code: str) -> Dict[str, Set[str]]:
        ...
        return {
            'uniforms': uniforms,
            'consts': consts,
            'potential_uses': var_uses
        }

    def check_undefined_vars(self, xml_path: Path, isf_path: Optional[Path] = None) -> FileAnalysisResult:
        """
